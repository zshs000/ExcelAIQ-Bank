...
## 8. 补充：真实 IP 透传还需要可信代理边界

> **2025-05-15 补记**：时隔许久，回头整理网关模块时再次检查 `ClientIpHeaderFilter`，又发现了一个更靠近入口边界的问题。

原来的过滤器会优先保留请求里已有的 `X-Forwarded-For` 和 `X-Real-IP`。这能解决“下游 auth 拿不到客户端 IP”的问题，但它默认这些头已经可信。实际请求里，这两个头并不天然可信，客户端也可以自己带上：
...
如果网关直接透传，下游验证码 IP 限流就会按伪造 IP 计数。攻击者只要不断换请求头里的 IP，就能绕开 IP 维度的发送次数限制。手机号维度限制仍然存在，但 IP 风控会失真。

所以真实 IP 透传不能只做“补头”，还要先确认这些头来自可信代理。更合理的入口规则是：

1. 先读取当前 TCP 直连来源，也就是 `remoteAddress`。
2. 如果 `remoteAddress` 不在可信代理配置中，就忽略 `X-Forwarded-For` 和 `X-Real-IP`，直接使用 `remoteAddress`。
3. 如果 `remoteAddress` 在可信代理配置中，再解析 `X-Forwarded-For`。
4. 多级代理场景下，从右往左跳过可信代理，取第一个不在可信代理列表里的 IP 作为客户端 IP。
...
如果 `127.0.0.1` 和 `10.0.0.0/8` 是可信代理，解析时会先跳过右侧的 `10.0.0.5`，然后把 `203.0.113.10` 识别为客户端 IP。最左侧的 `6.6.6.6` 可能是客户端在进入第一个可信代理前伪造出来的值，不能简单取最左边。

本次修复因此给网关增加 `gateway.client-ip.trusted-proxies` 配置，并让过滤器只把清洗后的单一客户端 IP 写给下游服务。开发环境默认信任本机代理地址；生产环境如果前面有 Nginx、SLB、CDN 或 Ingress，需要显式配置它们的出口 IP 或网段。
